...
export default function VerifierReviewListingsPage() {
  const qc = useQueryClient();
  const [error, setError] = useState<string | null>(null);

  const listingsQuery = useQuery({
    queryKey: ["verifier-review-listings"],
    queryFn: () =>
  ...
  const reviewMutation = useMutation({
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["verifier-review-listings"] });
    },
      ...
      <RequireVerifier>
        ...
        <DataTable<Listing>
          columns={[
            ...
            {
              header: "Harga",
              cell: (l) => <div className="text-slate-700">{formatRupiah(l.price ?? l.startingPrice)}</div>,
            },
            {
              header: "Aksi",
              className: "w-[240px]",
