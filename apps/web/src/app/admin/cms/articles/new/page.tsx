  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        authorName: form.authorName.trim() ? form.authorName : undefined,
      ...
      return apiFetchWithAuth<Article>("/admin/cms/articles", { method: "POST", body: JSON.stringify(payload) });
    },
    onSuccess: (created) => {
      router.replace(`/admin/cms/articles/${created.id}`);
    },
    onError: (e) => setError(e instanceof Error ? e.message : "Gagal membuat artikel"),
  });

  return (
    <AdminShell title="CMS • Buat Artikel">
      <RequireSuperAdmin>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input value={form.slug} onChange={(e) => setForm((p) => ({ ...p, slug: e.target.value }))} placeholder="contoh: panduan-kpr" />
              </div>
              <div className="space-y-2">
                <Label>Status</Label>
                <select
              ...
              <div className="space-y-2">
                ...
                <Input value={form.tags} onChange={(e) => setForm((p) => ({ ...p, tags: e.target.value }))} placeholder="kpr, smart living, legalitas" />
              </div>
              <div className="space-y-2 md:col-span-2">
              ...
              <div className="space-y-2 md:col-span-2">
                <Label>Content</Label>
                <Textarea value={form.content} onChange={(e) => setForm((p) => ({ ...p, content: e.target.value }))} className="min-h-[280px]" />
              </div>
            </div>
