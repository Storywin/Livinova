...
export default function AdminReviewProjectsPage() {
  const qc = useQueryClient();
  const [error, setError] = useState<string | null>(null);

  const projectsQuery = useQuery({
    queryKey: ["admin-review-projects"],
    queryFn: () => apiFetchWithAuth<Paginated<Project>>("/admin/review/projects?verificationStatus=pending&page=1&pageSize=50"),
  });
  ...
  const reviewMutation = useMutation({
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["admin-review-projects"] });
    },
